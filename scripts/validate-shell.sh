...
require_text .github/workflows/powder-run-testflight.yml 'date -u +%y%m%d%H%M'
require_text .github/workflows/powder-run-testflight.yml 'Confirm source matches passed iOS suite'
require_text .github/workflows/powder-run-testflight.yml 'git diff --quiet 4c02d5cda8519634eb6b24d95fd8b7a70a418d43 HEAD -- Shell ShellTests ShellUITests Packages PowderRunWidgets project.yml scripts/check-commerce-branding.sh scripts/validate-localizations.py scripts/app-store-connect-jwt.rb'
require_text .github/workflows/powder-run-testflight.yml 'UPLOAD POWDER RUN'
reject_text .github/workflows/powder-run-testflight.yml 'SCREENSHOT_BUILD'
...
if command -v sips >/dev/null 2>&1; then
  dimensions="$(sips -g pixelWidth -g pixelHeight Shell/Resources/Assets.xcassets/AppIcon.appiconset/PowderRun-1024.png 2>/dev/null)"
