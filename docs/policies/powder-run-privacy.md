...
You can enter customer and job identifiers, item descriptions and quantities, preparation checks, powder codes and lot references, operators, equipment and load identifiers, cure recipes, timestamps and part-metal readings, inspections, rework and scrap decisions, corrections, and related notes. Some entries can identify customers or workers. The app stores these records in its local application storage and uses them to display the shop board, validate the run workflow, and create reports. Presets are saved locally. We do not operate an account or an app server that receives these records.

If Live Activities are enabled on your device, an active oven load can appear on the Lock Screen or Dynamic Island, including the load identity, equipment name, run count, and a reading reminder. Avoid sensitive names in those fields if others can see your screen. The Live Activity is created on the device without remote push updates.

## Purchases and app settings
...
You can explicitly share a full JSON backup, job and oven-run CSV files, or a job PDF through the iOS share sheet. You choose the destination and should review its privacy settings. A JSON backup may contain all entered shop records, including personal information. Importing a backup replaces the local ledger after validation. No automatic Powder Run cloud backup is enabled. Apple's device-backup settings may include app data in an iCloud or computer backup; we do not control those settings or copies. To manage those copies, use your Apple device and backup settings.

Records stay on your device until you change or replace them, remove the app and its local data, or erase the device. Exported copies and device backups can persist separately until you delete them at their destinations. Uninstalling the app does not cancel an Apple subscription. We cannot delete a copy held in a file destination or Apple backup for you because we do not have access to it.
